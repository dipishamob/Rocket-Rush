  ...
  window.DOMElementHandler = class DOMElementHandler extends self.DOMHandler {
    ...
    _OnCreate(e) {
      ...
      if (this._autoAttach) {
        ...
        const parent =
          this.GetRuntimeInterface().GetHTMLWrapElement(actualHtmlIndex);
    ...
    _OnUpdatePosition(e) {
      ...
      const elemState = this._elementMap.get(e["elementId"]);
      const elem = elemState.GetElement();
      const iRuntime = this.GetRuntimeInterface();
      ...
      if (actualHtmlIndex !== elemState.GetActualHTMLIndex()) {
        elem.remove();
        const parent = iRuntime.GetHTMLWrapElement(actualHtmlIndex);
        parent.appendChild(elem);
    ...
    _OnHTMLLayersChanged() {
      ...
      for (const elemState of this._elementMap.values()) {
        const wantHtmlIndex = this.GetRuntimeInterface().GetAvailableHTMLIndex(
          elemState.GetWantHTMLIndex()
        );
        const actualHtmlIndex = elemState.GetActualHTMLIndex();
        if (
          wantHtmlIndex !== -1 &&
          actualHtmlIndex !== -1 &&
          ...
          elem.remove();
          const parent =
            this.GetRuntimeInterface().GetHTMLWrapElement(wantHtmlIndex);
          parent.appendChild(elem);
...
("use strict");
{
  ...
  async function CheckSupportsWorkerMode() {
	...
	self.addEventListener("message", () =>
	{
      ...
      const w = new Worker(URL.createObjectURL(workerScriptBlob), {
        get type() {
  ...
  window.RuntimeInterface = class RuntimeInterface {
    constructor(opts) {
      ...
      if (isSafari) this._useWorker = false;
      ...
      if (this.IsAnyWebView2Wrapper())
        self["chrome"]["webview"].addEventListener("message", (e) =>
          this._OnWrapperMessage(e.data, e["additionalObjects"])
        );
    ...
    IsAnyWebView2Wrapper() {
      return (
        this.IsWindowsWebView2() || this._exportType === "xbox-uwp-webview2"
      );
    }
    async _Init(opts) {
      if (this._useWorker) {
        const isWorkerModeSupported = await CheckSupportsWorkerMode();
    ...
    GetCssDisplayMode() {
      ...
      const standaloneExportTypes = new Set([
        "cordova",
        "nwjs",
        "macos-wkwebview",
      ]);
      if (standaloneExportTypes.has(exportType)) return "standalone";
      if (window.matchMedia("(display-mode: fullscreen)").matches)
        ...
        return "minimal-ui";
      else if (navigator["standalone"]) return "standalone";
      else return "browser";
    }
    ...
    async _InitWorker(opts, port2) {
      ...
      this._canvasLayers.push({ canvas, htmlWrap });
    ...
    _OnUpdateCanvasSize(e) {
      ...
      const heightPx = e["styleHeight"] + "px";
      const leftPx = e["marginLeft"] + "px";
      const topPx = e["marginTop"] + "px";
      for (const { canvas, htmlWrap } of this._canvasLayers) {
    ...
    _DoUpdateHTMLElementsZOrder() {
      ...
      for (const domHandler of this._domHandlers)
        if (domHandler instanceof window.DOMElementHandler) {
          ...
          if (elemStates) allElementStates.push(...elemStates);
        }
      allElementStates.sort((a, b) => {
        const a1 = a.GetActualHTMLIndex();
        const b1 = b.GetActualHTMLIndex();
        if (a1 !== b1) return a1 - b1;
        const a2 = a.GetHTMLZIndex();
        const b2 = b.GetHTMLZIndex();
        return a2 - b2;
      });
      let curHtmlIndex = 0;
      let s = 0,
    ...
    _DoUpdateHTMLElementsZOrderOnHTMLLayer(htmlIndex, arr) {
      ...
      const htmlWrap = this.GetHTMLWrapElement(htmlIndex);
      const existingChildren = Array.from(htmlWrap.children).filter((elem) =>
        newChildrenSet.has(elem)
      );
      let i = 0,
        len = Math.min(newChildren.length, existingChildren.length);
      for (; i < len; ++i) if (newChildren[i] !== existingChildren[i]) break;
      let j = i;
      for (; j < len; ++j) existingChildren[j].remove();
      j = i;
      for (; j < len; ++j) htmlWrap.appendChild(newChildren[j]);
    }
    _GetLocalRuntime() {
    ...
    _OnWrapperMessage(msg, additionalObjects) {
...
("use strict");
{
  ...
  const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
    ...
    _OnBeforeStartTicking() {
      ...
      if (this._iRuntime.GetExportType() === "cordova") {
        ...
        document.addEventListener("visibilitychange", () =>
          this._OnVisibilityChange(document.visibilityState === "hidden")
        );
      this._pageVisibilityIsHidden = !!(
        document.visibilityState === "hidden" || isCordovaPaused
      );
    ...
    _OnKeyEvent(name, e) {
      ...
      if (
        this._iRuntime.GetExportType() === "nwjs" &&
        e.key === "u" &&
        (e.ctrlKey || e.metaKey)
      )
        e.preventDefault();
...
("use strict");
{
  ...
  self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
    ...
    async _CreateAudioContext(e) {
      if (e["usePlayMusicAsSoundWorkaround"]) this._playMusicAsSound = true;
    ...
    *audioInstancesByEffectTag(tag) {
    ...
    TagsReady(tags) {
      const tagStr = (tags.length === 0 ? this._lastPlayedTags : tags).join(
        " "
      );
      const info = this._pendingTags.get(tagStr);
    ...
    async _AddEffect(e) {
      const type = e["type"];
      const tags = e.hasOwnProperty("tags") ? e["tags"] : [e["tag"]];
...
("use strict");
{
  self.C3AudioBuffer = class C3AudioBuffer {
    ...
    CreateInstance(tags) {
      if (this._api === "html5")
        return new self.C3Html5AudioInstance(this._audioDomHandler, this, tags);
      else
        return new self.C3WebAudioInstance(this._audioDomHandler, this, tags);
    }
    _Load() {}
    Load() {
...
("use strict");
{
  ...
  window.addEventListener("beforeinstallprompt", (e) => {
    ...
    if (browserDomHandler) browserDomHandler._OnBeforeInstallPrompt();
    return false;
  });
  ...
  const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
    ...
    Attach() {
      if (deferredInstallPromptEvent) this._OnBeforeInstallPrompt();
      else browserDomHandler = this;
...
("use strict");
{
  ...
  const HANDLER_CLASS = class LocalStorageDOMHandler extends self.DOMHandler {
    constructor(iRuntime) {
      super(iRuntime, DOM_COMPONENT_ID);
      this.AddRuntimeMessageHandlers([
        ["init", () => this._Init()],
    ...
    async _OnRequestPersistent() {
      try {
        ...
        console.error("[Construct] Error requesting persistent storage: ", err);
        return { isOk: false };
      }
    }
  };
...
document.addEventListener("DOMContentLoaded", function() {
	...
	video.id = "introVideo";
	video.width = 1580; // Set width for portrait mode
	video.height = 1100; // Set height for portrait mode
	video.autoplay = true; // Enable autoplay
	video.muted = true; // Mute the video to allow autoplay without user interaction
	video.style.position = "absolute"; // Position the video absolutely
	video.style.top = "50%"; // Center vertically
	video.style.left = "50%"; // Center horizontally
	video.style.transform = "translate(-50%, -50%)"; // Adjust for centering
	video.style.zIndex = "9999"; // Ensure the video is on top
	video.style.display = "block"; // Ensure the video is displayed
	video.controls = false; // Hide video controls
  
	const source = document.createElement("source"); // Create a source element for video
	source.src = videoSrc; // Set the video source
	source.type = "video/mp4"; // Set the video type
	video.appendChild(source); // Append the source to the video element
  
	/*const audio = document.createElement("audio"); // Create an audio element
	audio.id = "introAudio"; // Set an ID for the audio element
  ...
  */
	...
	video.addEventListener("ended", function() {
		...
		// Proceed with the game initialization
		startGame();
	});
